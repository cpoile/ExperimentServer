...
class PlayerLogic(globalId: String,
  ...
  def generateBlankRaceResults(): RaceResults = {
    val overallPoints = (for (i <- 0 until Lobby.settings.numTeams) yield {
      (i.toString -> 0)
    })
      .toMap
    val finishingPositions = (0 until Lobby.settings.numTeams).toList
    val changeInPoints = (for (i <- 0 until Lobby.settings.numTeams) yield {
      (i.toString -> 0)
    })
      .toMap
    ...
    val changeInRanking = (for (i <- 0 until Lobby.settings.numTeams) yield {
      (i.toString -> 0)
    })
      .toMap
    val trackNum = 0
    val historyOfWins = (for (i <- 0 until Lobby.settings.numTeams) yield {
      (i.toString -> 0)
    })
      .toMap
    RaceResults(
  ...
  def testHarnessCall(phase: TestingPhase.Value) {
    testHarnessCallbacks.filter(_._1 == phase).foreach {
      _._2()
    }
  }
