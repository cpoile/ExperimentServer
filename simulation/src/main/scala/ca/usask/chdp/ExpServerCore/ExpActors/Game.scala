...
class Game(var gameRecord: GameRecord) extends Actor with ActorLogging {
  ...
  val p1 = actorOf(
    Props(
      new PlayerLogic(
        gameRecord.p1globalId, gameRecord.gr_id, gameRecord.p1Role, gameRecord.manipulation, self)),
    gameRecord.p1globalId)
  val p2 = actorOf(
    Props(
      new PlayerLogic(
        gameRecord.p2globalId, gameRecord.gr_id, gameRecord.p2Role, gameRecord.manipulation, self)),
  ...
  val plrActor2globalId = Map[ActorRef, String](
    p1 -> gameRecord.p1globalId, p2 -> gameRecord.p2globalId)
  ...
  def setupRound(r: Int) {
    ...
    for (l <- adminListeners) {
      l ! GameRecordDAO.makeGameInfo(gameRecord.gr_id, Round(curRnd))
    }
  }
  ...
  case GetNextPartStep(part) => {
    ...
    if (pStepIter(part).hasNext) {
      ...
      log.error(
        "******************************* Ran out of partsteps to use, very bad. Shouldn't happen. ******************************")
    }
  }
  ...
  case BSentProject(b_metAllGoals) => {
    ...
    curRndRaceHistory = createRace(
      Lobby.settings.trackSeq(curRnd), helpBehsCount, Lobby.settings.numTeams,
      gameRecord.manipulation, curRnd)
    ...
    // send the message after we've recorded the record so that the actor system doesn't
    // go so far ahead in testing that we check the database before the gameRecord has been written.
    role2plrActor("RoleB") ! ISentProject(b_metAllGoals)
    role2plrActor("RoleA") ! PartnerSentProject
  }
  case ReadyForRace => {
    val lastRoundPts: immutable.Map[String, Int] = getLastRoundPointsIfAny(
      curRnd, Lobby.settings.numTeams, overallPoints.toMap)
    val changeInPts = getChangeInPoints(lastRoundPts, overallPoints(Round(curRnd)))
    val lastRoundRanking = getRoundRankingIfAny(
      curRnd - 1, Lobby.settings.numTeams, overallPoints.toMap)
  ...
  case RoundFinished(surveyName, questionReport) => {
    // if this was a survey round, record the answers.
    questionReport map { q =>
      surveyName map { sn =>
        val roleAndSurvey = plrActor2role(sender) + "_" + sn
        gameRecord = gameRecord.copy(
          surveyResults = gameRecord.surveyResults + (roleAndSurvey -> q))
      }
    }
