...
(function () {
   ...
   function animStepRun() {
      if (_.isEmpty(animStepQueue)) {
         ...
         console.log("Running animStep: " + nextStep.toString());
   ...
   /**
    * Variables that change based on Role.
    */
   var textVars = {
      step5_A: "Your job is to work on the parts and send the car to your teammate.",
      step5_B: "Your partner's job is to work on those parts and send the car to you.",
      step11_A: "Your partner takes your work and adds their parts.<br/>Your partner is in charge of: Suspension, Air Intake System, and Wing Aerodynamics",
      step11_B: "You will take this work and add your own parts.<br/>You are in charge of: Suspension, Air Intake System, and Wing Aerodynamics",
      step19_A: "After finishing their work, your partner sends the completed F1 car off to the race.<br>You will be able to watch the race to see how well your team performed.",
      step19_B: "After finishing your work, you will send the completed F1 car off to the race.<br>You will be able to watch the race to see how well your team performed."
   };
   ...
   /**
    * Customized animation for A or B.
    * @param role - "A" or "B"
    */
   myGuiders.anim_Part1 = function (role) {
      ...
      var step5 = function (callback) {
         $("<div id='tut_txtInst2' style='top: 550px; left: 50px' class='tutText'>" + textVars["step5_" + role] + "</div>").appendTo("#tutA5_container").fadeIn(animDelay, callback);
      };
      animStepAdd(step5);

      var step999 = function (callback) {
         $button.removeClass("disabled");
         $button.on("click", _.partial(myGuiders.bAnim_Part2, role));
         callback();
      };
      animStepAdd(step999);
   };

   myGuiders.bAnim_Part2 = function (role) {
      // disable next button, animation, then reenable next button with new function.
      var $button = $(this);
      $button.off("click");
      $button.addClass("disabled");
      ...
      var step11 = function (callback) {
         $("<div id='tut_txtInst3' style='top: 550px; left: 438px' class='tutText'>" + textVars["step11_" + role] + "</div>").appendTo("#tutA5_container").fadeIn(animDelay, function () {
            callback();
         });
      };
      animStepAdd(step11);
      ...
      var step999 = function (callback) {
         $button.removeClass("disabled");
         $button.on("click", _.partial(myGuiders.bAnim_Part3, role));
         callback();
      };
      animStepAdd(step999);
   };

   myGuiders.bAnim_Part3 = function (role) {
      var $button = $(this);
      $button.off("click");
      $button.addClass("disabled");
      ...
      var step19 = function (callback) {
         $("<div id='tut_txtInst4' style='top: 550px; left: 438px' class='tutText'>" + textVars["step19_" + role] + "</div>").appendTo("#tutA5_container").fadeIn(animDelay, callback);
      };
      animStepAdd(step19);

      var step999 = function (callback) {
         $button.removeClass("disabled");
         $button.on("click", myGuiders.bAnim_Part4);
         callback();
      };
      animStepAdd(step999);
   };

   myGuiders.bAnim_Part4 = function () {
      var $button = $(this);
      $button.off("click");
      $button.addClass("disabled");
